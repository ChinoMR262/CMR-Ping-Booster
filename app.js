/**
 * CMR PING BOOSTER - FRONTEND INTERACTION ENGINE
 */
...
// Simulador en vivo de telemetría para el HUD
function initLiveTelemetry() {
  ...
  setInterval(() => {
    // Oscilación realista de baja latencia con Nitro Activo
    ...
    // Actualizar strip de servidores de juegos
...
// Carga opcional y asíncrona de version.json
async function loadRemoteVersion() {
  try {
    ...
    if (data.android) {
      ...
      if (androidBadge) androidBadge.textContent = `v${data.android.versionName}`;
    }

    if (data.pc) {
      const pcBadge = document.getElementById('pc-version-badge');
      if (pcBadge) pcBadge.textContent = `v${data.pc.version}`;
    }
  } catch (err) {
    // Modo offline silencioso con valores por defecto del HTML
  }
}
